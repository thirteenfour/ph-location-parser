...
# UP TO CITY/MUNICIPALITY LEVEL ONLY.
# NO CODES; LIST ONLY.
...
for i in range(len(df.index)):
  ...
  try:
    ...
    # add new province/huc
      ...
      # json_data[current_rgn_name][current_provhuc_name] = {}
      json_data[current_rgn_name][current_provhuc_name] = []
      # check if huc or not
      if (pd.isnull(current_row["city_class"])):
        current_city_class = "not HUC"
      else:
        current_city_class = current_row["city_class"]
    # add new city/mun
      ...
      # check if not huc
      if not (current_city_class == "HUC"):
        json_data[current_rgn_name][current_provhuc_name].append(current_citymun_name)
    # else:
